...
//     (\`new ColoradoInformationMarketplaceSDK(...)\` / \`ColoradoInformationMarketplaceSDK.test(...)\` become
//     \`ColoradoInformationMarketplaceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ColoradoInformationMarketplaceSDK'
...
// rewrite real client construction (\`new ColoradoInformationMarketplaceSDK(...)\` and any
// \`ColoradoInformationMarketplaceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ColoradoInformationMarketplaceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ColoradoInformationMarketplaceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
