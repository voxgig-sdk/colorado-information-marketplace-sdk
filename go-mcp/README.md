# colorado-information-marketplace-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ColoradoInformationMarketplace SDK as
two agent tools — `colorado-information-marketplace_list` and `colorado-information-marketplace_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/colorado-information-marketplace-mcp)
make build

# 2. Provide credentials via the environment
export COLORADO_INFORMATION_MARKETPLACE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user colorado-information-marketplace \
  -- /absolute/path/to/colorado-information-marketplace-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./colorado-information-marketplace-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// colorado-information-marketplace_list: first page of records
{ "entity": "catalog" }
{ "entity": "catalog", "query": { } }

// colorado-information-marketplace_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/colorado-information-marketplace-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user colorado-information-marketplace \
     -- "$PWD"/dist/*/colorado-information-marketplace-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `colorado-information-marketplace_list` and `colorado-information-marketplace_load` tools now appear
   in new sessions. Ask the agent to *"list catalog using colorado-information-marketplace"*
   and it calls `colorado-information-marketplace_list` with `{"entity":"catalog"}`.
...
```sh
export COLORADO_INFORMATION_MARKETPLACE_APIKEY=sk_live_xxx            # API key
export COLORADO_INFORMATION_MARKETPLACE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `colorado-information-marketplace_list` tool
...
### Call the `colorado-information-marketplace_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `colorado-information-marketplace_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `colorado-information-marketplace_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COLORADO_INFORMATION_MARKETPLACE_APIKEY` | API key sent with every request. |
| `COLORADO_INFORMATION_MARKETPLACE_BASE` | Optional override of the API base URL. |
...
```sh
./colorado-information-marketplace-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"colorado-information-marketplace_load","arguments":{"entity":"catalog","query":{"id":1}}}}'
```
