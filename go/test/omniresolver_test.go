...
import (
	...
	omni "github.com/voxgig-sdk/colorado-information-marketplace-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ColoradoInformationMarketplaceError); is {
				out := map[string]any{
					"name":    "ColoradoInformationMarketplaceError",
...
func omniCtx(arg any, client *sdk.ColoradoInformationMarketplaceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ColoradoInformationMarketplaceSDK); is {
			client = live
