...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_LIVE") == "TRUE" ||
		os.Getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.ColoradoInformationMarketplaceSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.ColoradoInformationMarketplaceSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.ColoradoInformationMarketplaceError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.ColoradoInformationMarketplaceSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.ColoradoInformationMarketplaceError{Msg: msg, Code: code}
}
