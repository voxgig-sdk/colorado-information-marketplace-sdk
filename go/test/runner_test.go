...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("COLORADOINFORMATIONMARKETPLACE_TEST_LIVE") == "TRUE" ||
		os.Getenv("COLORADOINFORMATIONMARKETPLACE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("COLORADOINFORMATIONMARKETPLACE_TEST_EXPLAIN"); explain != "" {
