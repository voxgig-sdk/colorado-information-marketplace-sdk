# ColoradoInformationMarketplace SDK test runner
...
class ColoradoInformationMarketplaceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ColoradoInformationMarketplaceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ColoradoInformationMarketplaceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ColoradoInformationMarketplaceTestRunner.getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_LIVE")
        override = ColoradoInformationMarketplaceTestRunner.getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ColoradoInformationMarketplaceTestRunner.getenv(key)
        ...
        explain = ColoradoInformationMarketplaceTestRunner.getenv("COLORADO_INFORMATION_MARKETPLACE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ColoradoInformationMarketplaceTestRunner._test_control is not None:
            return ColoradoInformationMarketplaceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ColoradoInformationMarketplaceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ColoradoInformationMarketplaceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ColoradoInformationMarketplaceTestRunner.load_test_control()
...
def load_env_local():
    ColoradoInformationMarketplaceTestRunner.load_env_local()


def env_override(m):
    return ColoradoInformationMarketplaceTestRunner.env_override(m)


def entity_data(v):
    return ColoradoInformationMarketplaceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ColoradoInformationMarketplaceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ColoradoInformationMarketplaceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ColoradoInformationMarketplaceTestRunner.load_test_control()


def live_client_options():
    return ColoradoInformationMarketplaceTestRunner.live_client_options()


def live_delay_ms():
    return ColoradoInformationMarketplaceTestRunner.live_delay_ms()
