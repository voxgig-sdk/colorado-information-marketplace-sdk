...
require_relative 'ColoradoInformationMarketplace_types'
...
class ColoradoInformationMarketplaceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ColoradoInformationMarketplaceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ColoradoInformationMarketplaceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ColoradoInformationMarketplaceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ColoradoInformationMarketplaceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ColoradoInformationMarketplaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ColoradoInformationMarketplaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ColoradoInformationMarketplaceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ColoradoInformationMarketplaceError.new(
        "#{op}_allow",
        "ColoradoInformationMarketplaceSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ColoradoInformationMarketplaceError => err
    ...
    if fetched.is_a?(Hash)
      status = ColoradoInformationMarketplaceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ColoradoInformationMarketplaceError.new(
        "graphql_error", "ColoradoInformationMarketplaceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ColoradoInformationMarketplaceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
