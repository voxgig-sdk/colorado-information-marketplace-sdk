...
class CatalogEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = ColoradoInformationMarketplaceSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COLORADO_INFORMATION_MARKETPLACE_TEST_CATALOG_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def catalog_basic_setup(extra)
  ...
  entid_env_raw = ENV["COLORADO_INFORMATION_MARKETPLACE_TEST_CATALOG_ENTID"]
  ...
  if env["COLORADO_INFORMATION_MARKETPLACE_TEST_LIVE"] == "TRUE"
    ...
    client = ColoradoInformationMarketplaceSDK.new(Helpers.to_map(merged_opts))
  end
