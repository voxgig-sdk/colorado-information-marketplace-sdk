...
module ColoradoInformationMarketplaceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ColoradoInformationMarketplaceResponse.new({})
    ...
    if fetch_err
      ...
      response = ColoradoInformationMarketplaceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ColoradoInformationMarketplaceResponse.new(fetched)
    else
