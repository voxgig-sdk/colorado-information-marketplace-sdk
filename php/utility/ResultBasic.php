...
// ColoradoInformationMarketplace SDK utility: result_basic

class ColoradoInformationMarketplaceResultBasic
{
    public static function call(ColoradoInformationMarketplaceContext $ctx): ?ColoradoInformationMarketplaceResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof ColoradoInformationMarketplaceError) ? $result->err->msg : (string)$result->err;
