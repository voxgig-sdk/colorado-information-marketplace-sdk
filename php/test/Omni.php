...
//    only - a typed ColoradoInformationMarketplaceContext there would make every ctx
...
final class ColoradoInformationMarketplaceOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ColoradoInformationMarketplaceContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ColoradoInformationMarketplaceSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ColoradoInformationMarketplaceResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ColoradoInformationMarketplaceError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ColoradoInformationMarketplaceResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ColoradoInformationMarketplaceError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ColoradoInformationMarketplaceError(is_string($code) ? $code : '', $msg);
    }
