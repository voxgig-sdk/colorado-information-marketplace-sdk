...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new ColoradoInformationMarketplaceContext($ctxmap, null);
        ...
        if (isset($ctxmap['spec']) && is_array($ctxmap['spec'])) {
            $ctx->spec = new ColoradoInformationMarketplaceSpec($ctxmap['spec']);
        }

        // Handle result from JSON map
        if (isset($ctxmap['result']) && is_array($ctxmap['result'])) {
            $ctx->result = new ColoradoInformationMarketplaceResult($ctxmap['result']);
            if (isset($ctxmap['result']['err']) && is_array($ctxmap['result']['err'])) {
                $msg = $ctxmap['result']['err']['message'] ?? '';
                $ctx->result->err = new ColoradoInformationMarketplaceError('', $msg);
            }
        }

        // Handle response from JSON map
        if (isset($ctxmap['response']) && is_array($ctxmap['response'])) {
            $ctx->response = new ColoradoInformationMarketplaceResponse($ctxmap['response']);
    ...
    private static function fixctx(ColoradoInformationMarketplaceContext $ctx, ColoradoInformationMarketplaceSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ColoradoInformationMarketplaceError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ColoradoInformationMarketplaceError($code, $msg);
    }
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ColoradoInformationMarketplaceSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new ColoradoInformationMarketplaceOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ColoradoInformationMarketplaceSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ColoradoInformationMarketplaceError) ? $result->err->msg : (string)$result->err;
