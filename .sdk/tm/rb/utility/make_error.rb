...
module ColoradoInformationMarketplaceUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ColoradoInformationMarketplaceContext.new({}, nil)
    end
    op = ctx.op || ColoradoInformationMarketplaceOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ColoradoInformationMarketplaceResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ColoradoInformationMarketplaceError) ? err.msg : err.to_s
    ...
    sdk_err = ColoradoInformationMarketplaceError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ColoradoInformationMarketplaceError)
