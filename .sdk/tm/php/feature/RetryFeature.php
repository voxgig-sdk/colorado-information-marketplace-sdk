...
// ColoradoInformationMarketplace SDK retry feature
...
class ColoradoInformationMarketplaceRetryFeature extends ColoradoInformationMarketplaceBaseFeature
{
    ...
    public function init(ColoradoInformationMarketplaceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ColoradoInformationMarketplaceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ColoradoInformationMarketplaceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ColoradoInformationMarketplaceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
