...
boru-driven command-line client **and** interactive REPL for the ColoradoInformationMarketplace
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/colorado-information-marketplace-cli)
make build

# 2. See usage (words, entities, env vars)
./colorado-information-marketplace-cli --help
...
COLORADO_INFORMATION_MARKETPLACE_BASE=https://api.example.com ./colorado-information-marketplace-cli list catalog

# 6. No arguments -> interactive REPL
./colorado-information-marketplace-cli
colorado-information-marketplace> list catalog
colorado-information-marketplace> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/colorado-information-marketplace-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/colorado-information-marketplace-cli list catalog
   ```
...
Run with no arguments to open a REPL (prompt `colorado-information-marketplace>`). Each line is
evaluated as its own boru expression:

```text
$ ./colorado-information-marketplace-cli
colorado-information-marketplace> list catalog
colorado-information-marketplace> /help
colorado-information-marketplace> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/colorado-information-marketplace-cli`. |
...
(`./colorado-information-marketplace-cli <expr>`) and interactively (the REPL), and expressions compose the
